...
<?php if (!empty($iva)) : ?>
  <?php foreach ($iva as $valor) : ?>
    <?php
    $total_venta = model('kardexModel')->total_iva_apertura($apertura, $valor['iva']);
    $total_iva = model('kardexModel')->total_iva_producto_apertura($apertura, $valor['iva']);
    ?>
    <div class="row">
      <div class="col-8"></div>
...
<?php if (!empty($inc)) : ?>
  <?php foreach ($inc as $valor) : ?>
    ...
    <?php
    $total_venta_inc = model('kardexModel')->total_inc_apertura($apertura, $valor['inc']);
    $total_inc = model('kardexModel')->total_inc_producto_apertura($apertura, $valor['inc']);
    ?>
...
<?php if (!empty($iva)) : ?>
  <?php foreach ($iva as $valor) : ?>
    <?php
    $total_venta = model('kardexModel')->total_iva_fecha_apertura($apertura, $valor['iva']);
...
<?php if (!empty($inc)) : ?>
  <?php foreach ($inc as $valor) : ?>
    ...
    <?php
    $total_venta_inc = model('kardexModel')->total_inc_fecha_apertura($apertura, $valor['inc']);
