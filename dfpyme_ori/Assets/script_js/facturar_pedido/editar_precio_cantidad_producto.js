  ...
  $.ajax({
    ...
    success: function (resultado) {
      ...
      if (resultado.resultado == 0) {
        ...
        document.getElementById("editar_precioventa").value =
      ...
      if (resultado.resultado == 1) {
        $("#editar_precio_cantidad_producto_pedido").html(
          resultado.codigo_interno
        );
        $("#editar_precio_nombre_producto").html(resultado.descripcion);

        document.getElementById("editar_precio_id_tabla_producto").value =
          resultado.id_tabla_producto_pedido;
        document.getElementById("editar_precio_y_cantidad_venta").value =
          resultado.valor_unitario_formato;

        myModal = new bootstrap.Modal(
          document.getElementById(
            "editar_cantidad_y_precio_producto_pedidoFactura"
          ),
          {}
        );
        myModal.show();
 ...
 */
$(function () {
  $("#editar_cantidad_y_precio_producto_pedidoFactura").on(
    "shown.bs.modal",
    function (e) {
      $("#editar_precio_y_cantidad_venta").focus();
    }
  );
});

function editar_precio_multiplicarAgregar() {
  var total = 0;
  var cantidad = document.getElementById(
    "editar_precio_producto_pedido_cantidad"
  ).value;

  var valor_unidad = document.getElementById(
    "editar_precio_y_cantidad_venta"
  ).value;
  var valor_unitario = valor_unidad.replace(/[.]/g, "");
  if (
    cantidad == 0 ||
    cantidad < 0 ||
    cantidad == "" ||
    ...
    Swal.fire({
      ...
      title: "Error de dato en cantidad o precio unitario",
    });
  } else {
    ...
    document.getElementById("editar_precio_total").value = resultado;
  }
}
...
function actualizar_precio_cantidad() {
  var url = document.getElementById("url").value;
  var numero_pedido = document.getElementById("numero_de_facturacion").value;
  var id_tabla_producto = document.getElementById(
    "editar_precio_id_tabla_producto"
  ).value;
  var cantidad = document.getElementById(
    "editar_precio_producto_pedido_cantidad"
  ).value;
  var valor_unita = document.getElementById(
    "editar_precio_y_cantidad_venta"
  ).value;
  var valor_unitario = valor_unita.replace(/[.]/g, "");
  ...
  if (
    ...
    $.ajax({
      ...
      url:
        url +
        "/" +
        "edicion_eliminacion_factura_pedido/actualizar_precio_pedido",
      type: "POST",
      success: function (resultado) {
        var resultado = JSON.parse(resultado);
        if (resultado.resultado == 1) {
          $("#editar_cantidad_y_precio_producto_pedidoFactura").modal("hide");
...
const transaccion = document.querySelector("#editar_precio_y_cantidad_venta");
